...
abstract class Connection implements ConnectionInterface
{
    ...
    protected function getCacheKey(BaseQuery $query, string $method = ''): string
    {
        if (!empty($query->getOptions('key')) && empty($method)) {
            ...
            $table_name = is_array($table_name) ? current($table_name) : $table_name;
            $key        = 'think_' . $this->getConfig('database') . '.' . $table_name . '|' . $query->getOptions('key');
        } else {
     ...
     * @return string
     */
    protected function getCacheKey_00(BaseQuery $query, string $method = ''): string
    {
        halt($query->getTable());
        if (!empty($query->getOptions('key')) && empty($method)) {
